...
#     image_folder = "data/Images"  # <--- dossier où sont les images
    
#     with col1:
#         st.subheader("📸 Image")
#         try:
#             # Chemin de l'image dans le dataset
#             raw_path = sample['image_path'].replace("\\", "/")  # normaliser Windows -> Unix

#             # Supprimer un éventuel préfixe "data/Images/" pour éviter doublon
#             if raw_path.startswith("data/Images/"):
#                 raw_path = raw_path[len("data/Images/"):]

#             # Construire le chemin final
#             image_path = os.path.join(image_folder, raw_path)

#             if not os.path.exists(image_path):
#                 st.error(f"❌ Fichier introuvable: {image_path}")
#                 return
...
def init_s3_client():
    ...
    # Priorité aux variables d'environnement (Heroku)
